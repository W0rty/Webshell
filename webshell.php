                ...
                if(cmdComponents.includes('sudo') || cmdComponents.includes('su')){
                    ...
                    if(!cmdComponents.includes('cd')){
                        if(cmdComponents.includes('ls')){
                            $.get('/webshell.php?cmd='+cmdComponents+" "+currentPwd,function(data){
                            ...
                            if(cmdComponents.includes('cat')){
                                $.get('/webshell.php?cmd=cat '+currentPwd+"/"+cmdComponents.split(" ")[1],function(data){
                                ...
                                if(cmdComponents.includes('pwd')){
                                    ...
                                    $.get('/webshell.php?cmd='+cmdComponents).done(function(data){
                        ...
                        if(!cmdComponents.split(" ")[1].startsWith('/')){
                            $.get('/webshell.php?cmd=cd '+currentPwd+"/"+cmdComponents.split(" ")[1]+"; pwd").done(function(data){
                                data = data.replace('string','').replace(/\([0-9]*\)/g,'').replace('"','').replace('"','').replace(' ','\n');                                
                                Terminal.prototype.completePrompt = configs.getInstance().user + "@" + configs.getInstance().host + ":" + currentPwd + (configs.getInstance().root ? "#" : "$");
                                if(data===currentPwd && cmdComponents !== '.') Terminal.prototype.type('Invalid location.',function(){});
                                else Terminal.prototype.type('',function(){});
                                setCurrentPwd(data);
                            });
                        }else{
                            $.get('/webshell.php?cmd=cd '+cmdComponents.split(" ")[1]+"; pwd").done(function(data){
                                data = data.replace('string','').replace(/\([0-9]*\)/g,'').replace('"','').replace('"','').replace(' ','\n');
                                if(data===currentPwd) Terminal.prototype.type('Invalid location.',function(){});
                                else Terminal.prototype.type('',function(){});
                                setCurrentPwd(data);
                            });
                        }
